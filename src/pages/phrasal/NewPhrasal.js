...
const NewWord = ({ onClose }) => {
    ...
    const [newPhrasal, setNewPhrasal] = useState({
        phrasal: "",
        meanings: [
            {
                definition: "",
    ...
    const handleChange = (e, index) => {
        ...
        if (name === "phrasal") {
            ...
            const newMeanings = newPhrasal.meanings.map((meaning, i) => {
                if (i === index) {
    ...
    const handleDeleteMeaning = (index) => {
        setNewPhrasal({
            ...newPhrasal,
            meanings: newPhrasal.meanings.filter((_, i) => i !== index),
        });
    };

    const handleSubmit = async (e) => {
        const { phrasal, meanings } = newPhrasal;
        ...
        if (userRole === "admin") {
            const res = await fetch(`${process.env.REACT_APP_SERVER_URL}/phrasal/save`, {
                method: "POST",
                headers: {
    ...
    return (
        <div className="modal-content">
            <h3>
                <strong>Add New Phrasal Verb</strong>
            </h3>
            <div className="form-group">
                <div className="form-sub-group">
                    ...
                    <input
                        type="text"
                        name="phrasal"
                        value={newPhrasal.phrasal}
                        onChange={(e) => handleChange(e)}
                        required
                    />
                </div>
            </div>
            {newPhrasal.meanings.map((meaning, index) => (
